...
on one Cloudflare account except Supabase. Same DB/storage pattern as the
Seagonia admin app.

(We tried Clerk for auth briefly — its free tier watermarks the sign-in UI with
"Secured by Clerk", which isn't worth a paid plan just to remove for a private
family app. Plain Google OAuth via Supabase Auth has no such branding: Google's
own consent screen shows this app's name, not a vendor's.)
...
3. Add the four `VITE_*` env vars under **Settings → Environment variables**
   (`VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`, `VITE_UPLOAD_WORKER_URL`,
   `VITE_UPLOAD_SECRET`).
4. Deploy. You'll get a `*.pages.dev` URL (or attach a custom domain).
5. Once deployed, set `ALLOWED_ORIGIN` in `worker/wrangler.toml` to the real
   Pages URL and re-run `wrangler deploy` (from `worker/`).
